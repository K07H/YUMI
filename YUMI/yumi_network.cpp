...
QString YumiNetwork::sendGetRequestSync(const QString& url)
{
    QString result = "";
    QTimer timer;
    timer.setSingleShot(true);
    QEventLoop loop;
    YumiFileDownloader* fileDownloader = new YumiFileDownloader(url, this);
    connect(fileDownloader, SIGNAL(downloaded()), &loop, SLOT(quit()));
    connect(&timer, SIGNAL(timeout()), &loop, SLOT(quit()));
    timer.start(1000 * Config::Instance()->getRequestTimeout);
    loop.exec();
    if (timer.isActive())
    {
        if (fileDownloader->errorMsg().isEmpty())
        {
            result = QString(fileDownloader->downloadedData());
#if IS_DEBUG
            qDebug() << "Web request success:" << result;
#endif
        }
        else
        {
            result = ("ERROR: " + fileDownloader->errorMsg());
            qWarning().nospace() << "Unable to download string from " << url << ". Error message: " << fileDownloader->errorMsg();
        }
    }
    else
    {
        result = "ERROR: Request timed out.";
        qWarning().nospace() << "Unable to download string from " << url << ". Check internet connection.";
    }
