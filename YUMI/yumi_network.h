...
#ifndef __YUMI_NETWORK_H__
#define __YUMI_NETWORK_H__
...
class YumiNetwork : public QObject
{
    Q_OBJECT

private:
    YumiNetwork() = delete;
    YumiNetwork(const YumiNetwork& other) = delete;
    YumiNetwork& operator=(const YumiNetwork&) = delete;
    YumiNetwork(void* yumiPtr);

    static YumiNetwork* _instance;
...
#ifndef QT_NO_SSL
    void requestSslError(QNetworkReply* reply, QList<QSslError> errors);
#endif

public:
    ~YumiNetwork();
    static YumiNetwork* Init(void* yumiPtr);
    static YumiNetwork* Instance();

    void openLink(const QString& link, QWidget* msgBoxParent = NULL);
    void sendJsonPostRequest(const QString& url, const QString& jsonData);
    void sendGetRequest(const QString& url);
    QString sendGetRequestSync(const QString& url);

    bool isShareThemeRequest;
    bool isStartupGetLatestVersionRequest;
    bool isGetLatestVersionRequest;
};
