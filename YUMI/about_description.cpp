...
AboutDescription::AboutDescription(void* yumiPtr, QWidget* parent) : QWidget(parent)
{
    this->_yumiPtr = yumiPtr;

    this->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
    this->setMinimumWidth(ABOUT_DESCRIPTION_MIN_WIDTH);
    this->setContentsMargins(15, 0, 15, 0);

    this->_logo = new QLabel("");
    this->_logo->setPixmap(Assets::Instance()->appLogoImg);
    ...
    this->_description->setWordWrap(true);
    this->_description->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::MinimumExpanding);
    this->_description->setStyleSheet("QLabel { margin-left: 10px; padding: 0; " + Assets::Instance()->ABOUT_LABEL_STYLE + " }");

    this->_descriptionBox = new QHBoxLayout();
    this->_descriptionBox->addWidget(this->_logo);
    this->_descriptionBox->addWidget(this->_description);
    this->_descriptionBox->setAlignment(Qt::AlignLeft);
    this->_descriptionBox->setSpacing(0);
    this->_descriptionBox->setContentsMargins(0, 0, 0, 0);

    this->setLayout(this->_descriptionBox);
    this->adjustSize();
}

void AboutDescription::updateStyles()
{
    this->_description->setStyleSheet("QLabel { margin-left: 10px; padding: 0; " + Assets::Instance()->ABOUT_LABEL_STYLE + " }");
...
void AboutDescription::linkClicked(const QString& link)
{
    ((yumi*)_yumiPtr)->network->openLink(link, this);
}

#if IS_DEBUG && DEBUG_PAINTING
void AboutDescription::paintEvent(QPaintEvent*)
{
    qDebug() << "AboutDescription paint event!";
}
#endif
