...
private:
    ...
    GameLoader(void* yumi);

    bool isBepInExInstalled(const QDir& gameFolder);
    bool isUnityGame(const QDir& gameFolder, const QString& gameName);
    bool isDataFolderPresent(const QDir& gameFolder);
    void showLoadGameFolderResult(const int retval, const QString& folderPath);
    int loadGameFolder(const QString& folderPath);

    static GameLoader* _instance;

    void* _yumiPtr;

public:
    static GameLoader* Init(void* yumi);
    static GameLoader* Instance();

    int addGameFolder(const QString& folderPath);
    int openGameFolder();
};
