...
#ifndef __MODS_LIST_H__
#define __MODS_LIST_H__
...
class ModsList : public QWidget
{
    Q_OBJECT

private:
    ModsList() = delete;
    ModsList(QWidget* parent) = delete;

    void* _parent;
    QVBoxLayout _mainLayout;
    QList<QPushButton*> _installedMods;
    QString _disabledPrefix;

    void setButtonStyle(QPushButton* btn, const bool selected, const bool disabled);
    void refreshButtonsStyles();

private slots:
    void modButtonClicked();

public:
    explicit ModsList(const QString& disabledPrefix, QWidget* parent = NULL);
    ~ModsList();
