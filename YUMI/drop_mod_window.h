...
#ifndef __DROP_MOD_WINDOW_H__
#define __DROP_MOD_WINDOW_H__
...
#define MAX_FILES_DROP_AT_ONCE 32

class DropModWindow : public QWidget
{
    Q_OBJECT

public:
    explicit DropModWindow(void* yumiPtr, QWidget* parent = NULL);
...
private:
    DropModWindow() = delete;
    DropModWindow(QWidget* parent) = delete;
    QStringList getDroppedFilesList(const QMimeData* mimeData);
    void processDroppedMods();

    void* _yumiPtr;
    QColor _areaColor;
    QColor _areaColorOpaque;
    QColor _dropModHereColor;
    QFont* _dropModHereFont;
    WindowTitle* _title;
    QLabel* _methodLabel;
    QLabel* _alternateMethodLabel;
    ...
    bool _drawTransparentArea;
    QString _modsFolderPath;
    QString _gameName;
    QStringList _modsToProcess;
    QFuture<void> _processDroppedMods;
};
