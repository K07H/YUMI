...
int main(int argc, char *argv[])
{
    ...
    QCoreApplication::setOrganizationName("LC-IT Solutions");
    QCoreApplication::setOrganizationDomain("lc-it-solutions.com");
    QCoreApplication::setApplicationName("YUMI - Your Unity Mods Installer");

    QCommandLineParser parser;
    parser.setApplicationDescription("YUMI - Your Unity Mods Installer");
    parser.addHelpOption();
    parser.addVersionOption();
    parser.addPositionalArgument("folder", QCoreApplication::translate("Main", "The game folder to register.", "Command line argument description text"));
    parser.process(a);

    QString folderToLoad = parser.positionalArguments().value(0, QString(""));
    ...
    int screenNo = QApplication::desktop()->screenNumber(&mw);
    QList<QScreen*> screens = QGuiApplication::screens();
    int len = screens.count();
    if (len > screenNo && screens[screenNo] != NULL)
        availableGeometry = screens[screenNo]->availableGeometry();
#else
