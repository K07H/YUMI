...
bool ModsLoader::isBepInExInstalled(GameInfo* gameInfo, const QDir& gameFolder)
{
    if (!gameFolder.exists())
        return false;
    ...
    if (bepInExFiles == NULL)
    {
        qCritical() << "BepInEx files for this game are not included in this YUMI version.";
        return false;
    }
...
bool ModsLoader::copyRecursively(QString sourceFolder, QString destFolder)
{
...
void ModsLoader::extractIncludedMods()
{
    QString modsFolderPath = Utils::toUnixPath(yumi::appPath + QDir::separator() + "mods");
    QDir modsFolder(modsFolderPath);
    if (!modsFolder.exists())
    {
        qInfo().nospace() << "No mods requiring installation were found in the \"mods\" folder.";
        return;
    }
    ...
    QString tempBepInExDirPath = Utils::toUnixPath(yumi::appPath + QDir::separator() + "temp_bepinex");
    QDir tempBepInExDir(tempBepInExDirPath);
    if (tempBepInExDir.exists())
        if (!tempBepInExDir.removeRecursively())
            qWarning().nospace() << "Could not remove folder " << tempBepInExDirPath << ".";
    if (!yumiDir.mkdir("temp_bepinex"))
        qWarning().nospace() << "Could not create temporary folder " << tempBepInExDirPath << ".";
    if (!tempBepInExDir.exists())
    {
        qCritical().nospace() << "Temporary BepInEx folder creation failed.";
        return;
    }
    if (!tempBepInExDir.mkdir("config"))
        qWarning().nospace() << "Could not create temporary BepInEx config folder.";
    QString tempBepInExConfigDirPath = Utils::toUnixPath(tempBepInExDirPath + QDir::separator() + "config");
    QDir tempBepInExConfigDir(tempBepInExConfigDirPath);
    if (!tempBepInExConfigDir.exists())
    {
        qWarning().nospace() << "Unable to create temporary BepInEx config folder.";
        return;
    }
    if (!tempBepInExDir.mkdir("patchers"))
        qWarning().nospace() << "Could not create temporary BepInEx patchers folder.";
    QString tempBepInExPatchersDirPath = Utils::toUnixPath(tempBepInExDirPath + QDir::separator() + "patchers");
    QDir tempBepInExPatchersDir(tempBepInExPatchersDirPath);
    if (!tempBepInExPatchersDir.exists())
    {
        qWarning().nospace() << "Unable to create temporary BepInEx patchers folder.";
        return;
    }
    if (!tempBepInExDir.mkdir("plugins"))
        qWarning().nospace() << "Could not create temporary BepInEx plugins folder.";
    QString tempBepInExPluginsDirPath = Utils::toUnixPath(tempBepInExDirPath + QDir::separator() + "plugins");
    QDir tempBepInExPluginsDir(tempBepInExPluginsDirPath);
    if (!tempBepInExPluginsDir.exists())
    {
        qWarning().nospace() << "Unable to create temporary BepInEx plugins folder.";
        return;
    }
    ...
    if (modsList.count() <= 0)
    {
        ...
        qInfo().nospace() << "Found " << modsList.count() << " mods to extract.";

    QString tempModsDirPath = Utils::toUnixPath(yumi::appPath + QDir::separator() + "temp_mods");
    if (!yumiDir.mkdir("temp_mods"))
        qWarning().nospace() << "Could not create temporary folder " << tempModsDirPath << ".";
    QDir tempModsDir(tempModsDirPath);
    if (!tempModsDir.exists())
    {
        qCritical().nospace() << "Temporary mods folder creation failed.";
        return;
    }
    try
    {
                ...
                if (tempModDir.exists()) {
                    if (ZipfilesManager::Instance()->unzip(modFilePath, tempModDirPath)) {
                        ...
                        if (actualModFolder.exists()) {
                            modDirectories = actualModFolder.entryList(QDir::Dirs | QDir::NoDotAndDotDot | QDir::NoSymLinks);
                            ...
                            if (nbModDirectories > 0) {
                                for (int i = 0; i < nbModDirectories; i++) {
                                    if (modDirectories[i].compare("config") == 0) {
                                        QString modConfigFolderPath = Utils::toUnixPath(actualModFolderPath + QDir::separator() + "config");
                                        if (!copyRecursively(modConfigFolderPath, tempBepInExConfigDirPath))
                                            qWarning().nospace() << "Failed to extract config files for mod " << modFileNameNoExt << ".";
                                    } else if (modDirectories[i].compare("patchers") == 0) {
                                        QString modPatchersFolderPath = Utils::toUnixPath(actualModFolderPath + QDir::separator() + "patchers");
                                        if (!copyRecursively(modPatchersFolderPath, tempBepInExPatchersDirPath))
                                            qWarning().nospace() << "Failed to extract patchers files for mod " << modFileNameNoExt << ".";
                                    } else if (modDirectories[i].compare("plugins") == 0) {
                                        QString modPluginsFolderPath = Utils::toUnixPath(actualModFolderPath + QDir::separator() + "plugins");
                                        if (!copyRecursively(modPluginsFolderPath, tempBepInExPluginsDirPath))
                                            qWarning().nospace() << "Failed to extract plugins files for mod " << modFileNameNoExt << ".";
                                    }
                                }
                            } else {
                                qWarning().nospace() << "The archive organisation is not correct for mod file " << modFilePath << ".";
                            }
                        } else {
                            qWarning().nospace() << "The archive organisation is not correct for mod file " << modFilePath << ".";
                        }
                    ...
                    qWarning().nospace() << "Could not create mod temporary folder " << tempModDirPath << ".";
                }
            } else {
                qWarning().nospace() << "Could not create temporary folder " << modFileNameNoExt << " for mod file " << modFileName << ".";
            }
        }
    }
    catch (const std::exception& ex)
    {
        qWarning().nospace() << "Exception caught while extracting mods. Exception: " << ex.what();
    }
    catch (...)
    {
        qWarning().nospace() << "Exception caught while extracting mods.";
    }

    qInfo().nospace() << "Mods have been extracted.";
    if (!tempModsDir.removeRecursively())
        qWarning().nospace() << "Failed to remove temporary mods folder";
}

bool ModsLoader::installBepInEx(void* yumiPtr, void* gameDetails, GameInfo* game, const bool forEveryone)
{
    if (yumiPtr == NULL || game == NULL || game == NULL)
    {
    ...
    if (tempBepInExDir.exists())
    {
        qInfo().nospace() << "Installing mods.";
        try
        {
            QString bepInExDestPath = Utils::toUnixPath(game->path + QDir::separator() + "BepInEx");
            if (!copyRecursively(tempBepInExDirPath, bepInExDestPath))
                qWarning().nospace() << "Some mods failed to install.";
        }
        catch (const std::exception& ex)
        {
            qWarning().nospace() << "Exception caught while installing mods. Exception: " << ex.what();
        }
        catch (...)
        {
            qWarning().nospace() << "Exception caught while installing mods.";
        }
        if (!tempBepInExDir.removeRecursively())
            qWarning().nospace() << "Failed to remove temporary BepInEx folder at " << tempBepInExDirPath << ".";
        qInfo().nospace() << "Mods have been installed.";
    }
