...
WindowTitle::WindowTitle(const QString& title, QWidget* parent) : QWidget(parent)
{
    this->_title = new QLabel(title, this);
    this->_title->setStyleSheet("QLabel { padding: 8px; " + Assets::Instance()->BOLD_LABEL_STYLE + " }");
    ...
    this->setStyleSheet("WindowTitle { " + Assets::Instance()->DEFAULT_WINDOW_TITLE_STYLE + " }");
    this->setLayout(this->_titleLayout);
}

void WindowTitle::updateStyles()
{
    this->_title->setStyleSheet("QLabel { padding: 8px; " + Assets::Instance()->BOLD_LABEL_STYLE + " }");
    this->setStyleSheet("WindowTitle { " + Assets::Instance()->DEFAULT_WINDOW_TITLE_STYLE + " }");
...
{
#if IS_DEBUG && DEBUG_PAINTING
    qDebug() << "WindowTitle paint event!";
#endif
