...
#define DISPLAY_FILE_TREES false
#define DEBUG_LOG_FILENAME "YUMI_Debug.log"

class DebugLogs : public QWidget
{
    Q_OBJECT

public:
    explicit DebugLogs(void* yumiPtr, QWidget* parent = NULL);
...
private:
    DebugLogs() = delete;
    DebugLogs(QWidget* parent) = delete;
    DebugLogs(const DebugLogs& other) = delete;
    DebugLogs& operator=(const DebugLogs&) = delete;
    QString getFullLogs();
    void refreshSelectedGameData(const QString& selectedGame);

    void* _yumiPtr;
    WindowTitle* _title;

    QLabel* _yumiLogsTitle;
    QLabel* _yumiLogsLabel;
    QScrollArea* _yumiLogsScroller;

#if DISPLAY_FILE_TREES
    QLabel* _yumiFileTreeTitle;
    QLabel* _yumiFileTreeLabel;
    QScrollArea* _yumiFileTreeScroller;
#endif

    QLabel* _selectGameLabel;
    QComboBox* _selectGameDropdown;
    QHBoxLayout* _selectedGameLayout;

    QLabel* _gameLogsTitle;
    QLabel* _gameLogsLabel;
    QScrollArea* _gameLogsScroller;

#if DISPLAY_FILE_TREES
    QLabel* _gameFileTreeTitle;
    QLabel* _gameFileTreeLabel;
    QScrollArea* _gameFileTreeScroller;
#endif

    QPushButton* _copyToClipboardButton;
...
private slots:
    ...
    void exportLogs();
    void selectedGameChanged(QString selectedGame);
};
