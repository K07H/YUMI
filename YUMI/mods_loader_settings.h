...
#ifndef __MODS_LOADER_SETTINGS_H__
#define __MODS_LOADER_SETTINGS_H__
...
class ModsLoaderSettings : public QWidget
{
    Q_OBJECT

private:
    ModsLoaderSettings() = delete;
    ModsLoaderSettings(QWidget* parent) = delete;
    QString getExeTypeComboBoxStr(int exeType);
    int getExeTypeComboBoxInt(const QString& exeType);
...
public:
    explicit ModsLoaderSettings(void* yumiPtr, GameInfo* gameInfo, void* gameDetails, QWidget* parent = NULL);
    void doShowAt(const QPoint& center);
    void updateStyles();

    WindowTitle* _title;
    QLabel* _gameExePathLabel;
    QLabel* _gameExePath;
    QPushButton* _gameExePathBtn;
    QVBoxLayout* _gameExePathLayout;
    QLabel* _gameExeTypeLabel;
    QComboBox* _gameExeType;
    QHBoxLayout* _gameExeTypeLayout;
